...
write.csv(MSE.dat, file = "MSEvsEpsilonValuesSocial.csv")

################################################################################

# Run the following lines for re-plotting from old saved data.

# MSE.dat <- read.csv("MSEvsEpsilonValuesSocial.csv")[,-1]
...
# saving the plot
ggsave(plot = MSE.plot, filename = "MSEvsEpsilonSocial.pdf", device = "pdf",
       width = 4, height = 3)
