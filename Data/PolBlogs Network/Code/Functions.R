...
sim.beta.est <- function(n.rep, beta.true, N, J.N,
  ...
  #                                       beta estimates
  #                        If priv = T -> a n.rep x 2 matrix with each row 
  #                                       (non-private, private) estimates of
  #                                       beta
  ##############################################################################
  ...
  out.beta.hat <- foreach(k=1:n.rep, .combine = rbind, .export = ls(envir=globalenv())) %dopar% {
    ...
    if (priv == T){
      ...
      return (c(beta.hat, beta.hat.priv))
